...
-- Rebuild the combined sound list from builtins + scanned + manual custom sounds
function CP:RebuildSoundList()
  ...
  -- Add auto-scanned sounds (from soundlist.lua, generated by scan.py)
  if self.scannedSounds then
    for _, filename in ipairs(self.scannedSounds) do
      if not seen[filename] then
        seen[filename] = true
        table.insert(self.critSounds, {
          name = "* " .. filename:gsub("%.ogg$", ""),
          soundID = nil,
  ...
  -- Add manually registered custom sounds (from /cp addsound)
